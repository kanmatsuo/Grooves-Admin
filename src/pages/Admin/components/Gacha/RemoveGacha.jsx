...
const Component = forwardRef((props, ref) => {
    ...
    async function loadBlockchainData() {
        ...
        try {
            ...
            setSource(items.filter(c => c.status.includes(6)));
        } catch (err) {
    ...
    const removeFromGachaList = async () => {
        ...
        try {
            ...
            var tx = await gachaContract.methods.removeGachaList(idsForCreate).send({ from: account });
            ...
            for (var i = 0; i < count; i++) {
                ...
                var message = "#" + idsForCreate[i] + " Token Successfully removed From Gacha list!"
                notify(message);
            }
            ...
            <div className="card">
                ...
                <div className="flex justify-content-center">
                    <div className='flex justify-content-center my-5 pt-3'>
                        <Button label="REMOVE GACHA" onClick={removeFromGachaList} loading={loading} className='gradientBtn02' icon="pi pi-check" />
                    </div>
                </div>
            </div>
        </>
