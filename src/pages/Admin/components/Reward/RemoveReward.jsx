...
const Component = forwardRef((props, ref) => {
    ...
    async function loadBlockchainData() {
        ...
        try {
            ...
            setSource(items.filter(c => c.status.includes(8)));
        } catch (err) {
    ...
    const itemTemplate = (item) => {
        return (
            <div className="flex flex-wrap p-2 align-items-center gap-3">
                <Avatar size="xlarge" className="p-overlay-badge" image={item.image}>
                    <Badge value={item.id} />
                </Avatar>
    ...
    const removeFromRewardList = async () => {
        ...
        try {
            ...
            var tx = await rewardContract.methods.removeAwardList(idsForCreate).send({ from: account });
            ...
            for (var i = 0; i < count; i++) {
                ...
                var message = "#" + idsForCreate[i] + " Token Successfully removed From Reward list!"
                notify(message);
            }
            ...
            <div className="card">
                ...
                <div className="flex justify-content-center">
                    <div className='flex justify-content-center my-5 pt-3'>
                        <Button label="REMOVE REWARD" onClick={removeFromRewardList} loading={loading} className='gradientBtn02' icon="pi pi-check" />
                    </div>
                </div>
            </div>
        </>
