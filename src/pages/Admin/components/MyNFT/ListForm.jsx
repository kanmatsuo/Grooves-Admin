...
const Component = forwardRef((props, ref) => {
    ...
    const handleCreate = async (data) => {
        try {
            setLoading(true)
            var tx = await listingContract.methods.createListedToken(tokenId, paytype, Web3.utils.toWei(String(data.price))).send({ from: account })
            var currency = paytype == '1' ? "GRVC" : "BNB";
            var message = "#" + tokenId + "NFT listed as " + data.price + " " + currency;
            notify(message);
            history({
                id: tokenId,
                from: account,
            ...
            <div className="gap-5 flex flex-column w-12 mb-4">
                <form onSubmit={formik.handleSubmit}>
                    ...
                    <div className='flex justify-content-center my-3 pt-3'>
                        <Button loading={loading} label="CREATE" type="submit" className='gradientBtn02' icon="pi pi-check" />
                    </div>
                </form>
            </div>
        </div >
